...
const state = {
  ...
  timer: {
    ...
    // Se pone en true cuando el jefe arranca el cronómetro, y se mantiene
    // así aunque el cronómetro luego se congele (revelado, pausa manual):
    // es la señal de que el intento ya empezó, no de que siga corriendo.
    // La votación se bloquea mientras esto sea false (ver registerVote).
    // Un reset (automático, manual, o el botón "reiniciar" del jefe) lo
    // vuelve a poner en false.
    startedThisAttempt: false,
    // Cuando se revela la decisión, el cronómetro queda detenido y el jefe
    // no puede volver a arrancarlo durante los 15 s del auto-reset. Esto
    // evita que se reanude por accidente justo después de una decisión.
    autoResetLockUntil: null,
  },
}
...
function registerVote(judgeId, value) {
  ...
  // No se puede votar hasta que el jefe arranque el cronómetro: es la
  // señal de que el intento ya empezó. Sin este candado un juez podría
  // adelantar su voto antes del levantamiento.
  if (!state.timer.startedThisAttempt) return
  ...
  if (countVotesIn() === 3) {
    ...
    state.timer.autoResetLockUntil = Date.now() + config.autoResetSeconds * 1000
...
function resetAttempt() {
  clearAutoReset()
  state.timer.autoResetLockUntil = null
  ...
  // el cronómetro a la duración completa, junto con las luces, y vuelve a
  // bloquear la votación hasta que el jefe lo arranque de nuevo.
  state.timer.running = false
  state.timer.startedAt = null
  state.timer.frozenRemainingMs = null
  state.timer.startedThisAttempt = false

  notifyChange()
}

function isTimerStartBlocked() {
  return (
    state.phase === PHASE.REVEALED &&
    state.timer.autoResetLockUntil !== null &&
    Date.now() < state.timer.autoResetLockUntil
  )
}

function startTimer() {
  if (isTimerStartBlocked()) return

  state.timer.running = true
  state.timer.startedAt = Date.now()
  state.timer.frozenRemainingMs = null
  state.timer.startedThisAttempt = true
  notifyChange()
}
...
function getStateFor(role, judgeId) {
  const base = {
    ...
    timer: {
      ...
      started: state.timer.startedThisAttempt,
      startBlocked: isTimerStartBlocked(),
    },
  }
